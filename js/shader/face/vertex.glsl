...
const vec2 DISPLACEMENT0 = vec2(0.0 ,0.15);
const float RADIUS0 = 0.15;

// deformation 0 parameters:
const vec2 TEARPOINT1 = vec2(0.2, 0.1);
const vec2 DISPLACEMENT1 = vec2(0.0,0.15);
const float RADIUS1 = 0.15;
...
void main() {
  ...
  float deformFactor0 = 1.0-smoothstep(0.0, RADIUS0, distance(TEARPOINT0, position.xy));
  positionDeformed.xz += deformFactor0*DISPLACEMENT0;

  // apply deformation 1: right eye
  float deformFactor1 = 1.0-smoothstep(0.0, RADIUS0, distance(TEARPOINT1, position.xy));
  positionDeformed.xz += deformFactor1*DISPLACEMENT0;
