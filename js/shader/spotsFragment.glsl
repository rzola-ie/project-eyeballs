...
void main() {
  ...
  // vec2 stretchedUv = vec2((vUv.x + 0.1) * 0.8, vUv.y);

    vec2 leftUv = vec2(vUv.x-amount,vUv.y);
    if(leftUv.x > 0.9) discard;
    vec3 colLeft;
    colLeft.r = texture( feed, leftUv ).r;
    colLeft.g = texture( feed, leftUv ).g;
    colLeft.b = texture( feed, leftUv ).b;
    colLeft *= (1.0 - amount * 0.5);

    vec2 rightUv = vec2(vUv.x+amount,vUv.y);
    if(rightUv.x < 0.1) discard;
    vec3 colRight;
    colRight.r = texture( feed, rightUv ).r;
    colRight.g = texture( feed, rightUv ).g;
    colRight.b = texture( feed, rightUv ).b;

    colRight *= (1.0 - amount * 0.5);

    vec3 colFinal;
    colFinal.r += mix(colLeft.r, colRight.r, 0.5);
    colFinal.g += mix(colLeft.g, colRight.g, 0.5);
    colFinal.b += mix(colLeft.b, colRight.b, 0.5);
